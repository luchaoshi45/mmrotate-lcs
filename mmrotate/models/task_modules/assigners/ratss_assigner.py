...
def rbbox2hbbox_center_distance(bboxes1: Tensor, bboxes2: Tensor) -> Tensor:
    ...
    bboxes = bboxes1.convert_to('hbox').tensor
    priors = get_box_tensor(bboxes2)
...
class RATSSAssigner(BaseAssigner):
    ...
    def assign(
        ...
        distances = rbbox2hbbox_center_distance(gt_bboxes, priors)
        ...
        priors_cx = (priors.tensor[:, 0] + priors.tensor[:, 2]) / 2.0
        priors_cy = (priors.tensor[:, 1] + priors.tensor[:, 3]) / 2.0
        ...
        gt_bboxes = gt_bboxes.convert_to('hbox')
        l_ = ep_priors_cx[candidate_idxs].view(-1, num_gt) - gt_bboxes.tensor[:, 0]
        t_ = ep_priors_cy[candidate_idxs].view(-1, num_gt) - gt_bboxes.tensor[:, 1]
        r_ = gt_bboxes.tensor[:, 2] - ep_priors_cx[candidate_idxs].view(-1, num_gt)
        b_ = gt_bboxes.tensor[:, 3] - ep_priors_cy[candidate_idxs].view(-1, num_gt)
