...
def IGIoU(k,x):
    ...
    # print(f"min_val:{sys.float_info.min}")
    # if (1+x)<0:
        # x=sys.float_info.min-1
...
def smooth_giou_loss(pred,
    ...
    # print(f"1--smooth_giou_loss: {pGIOU.shape}")

    GIOU=torch.diagonal(pGIOU, 0)

    # print(f"2--smooth_giou_loss: {GIOU.shape}")

    # loss=torch.where(GIOU<beta, IGIoU(k,GIOU)-0.5*IGIoU(k,beta), 0.5*(IGIoU(k,GIOU)**2)/IGIoU(k,beta))
    ...
    # loss=GIOU.apply_(lambda item: IGIoU(k,item)-0.5*IGIoU(k,beta) if item<beta else 0.5*(IGIoU(k,item)**2)/IGIoU(k,beta) )
...
class SmoothGIoULoss(nn.Module):
    ...
    def forward(self,
        ...
        loss = 0.2 * smooth_giou_loss(pred,target,weight,reduction=reduction,avg_factor=avg_factor,**kwargs)+ 0.8*smooth_l1_loss(pred,target)
        return loss
