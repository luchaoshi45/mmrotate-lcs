...
def box_convex_hull_tensor(boxes1, boxes2):
    ...
    hull = hull_abc[:, :, 0] * hull_bca[:, :, 1] - \
           hull_abc[:, :, 1] * hull_bca[:, :, 0]
...
def generalized_box_iou(boxes1, boxes2):
    ...
    """
    hull = box_convex_hull_tensor(boxes1, boxes2)

    return iou - (hull - union) / hull


def IGIoU(k,x):
    # return k*torch.log(torch.tensor(2.0))-k*torch.log(torch.add(torch.clamp(torch.tensor(x), min=torch.tensor(-1+1e-5), max=torch.tensor(1.0)),1.0)) #.to(x.device)
    # return k*torch.log(torch.tensor(2.0))-k*torch.log(torch.add(torch.clamp(torch.tensor(x), min=-1+1e-5, max=1.0), 1.0)) #.to(x.device)
    if isinstance(x, float):
        x1 = k * torch.log(torch.tensor(2.0))
        x222 = torch.clamp(torch.tensor(x), min=-1 + 1e-5, max=1.0)
        x22 = torch.add(x222, 1.0)
        x2 = k * torch.log(x22)

    elif isinstance(x, torch.Tensor):
        device = x.device
        x1 = k * torch.log(torch.tensor(2.0).to(device))
        # x222 = torch.clamp(torch.tensor(x), min=-1 + 1e-5, max=1.0)
        x222 = torch.clamp(x, min=-1 + 1e-5, max=1.0)
        x22 = torch.add(x222, 1.0)
        x2 = k*torch.log(x22)
    else:
        pass
...
def smooth_giou_loss(pred,
    ...
    two parameters: k and beta

    IGIoU(k,x)=k*ln2-k*ln(1+x)

    GIoU(x)

    Smooth_GIoU(k,beta,GIoU(x))=
        IGIoU(k,GIoU(x))-0.5*IGIoU(k,beta) if GIoU(x)<beta
        0.5*(IGIoU(k,GIoU(x))^2) / IGIoU(k,beta) if GIoU(x)>=beta
    ...
    Returns:
        loss (torch.Tensor)
    """
    assert pred.shape[-1] == 5, "pred bboxes should be (x1, y1, x2, y2, score)"
    assert target.shape[-1] == 5, "target bboxes should be (x1, y1, x2, y2, score)"

    qbox_pred=rbox2qbox(pred)
    qbox_target=rbox2qbox(target)

    pGIOU=generalized_box_iou(qbox_pred, qbox_target)
    GIOU=torch.diagonal(pGIOU, 0)

    # from mmrotate.models.losses import ConvexGIoULoss
    # from mmrotate.structures.bbox import r
    # giou_loss = ConvexGIoULoss()
    # giou_loss(qbox_pred, qbox_target, reduction=None)

    loss = []
    for item in GIOU:
        if item < beta:
            loss.append(IGIoU(k,item)-0.5*IGIoU(k,beta))
        else:
            loss.append(0.5*(IGIoU(k,item)**2)/IGIoU(k,beta))
    loss = torch.stack(loss)

    # loss=torch.tensor([IGIoU(k,item)-0.5*IGIoU(k,beta) if item<beta else 0.5*(IGIoU(k,item)**2)/IGIoU(k,beta) for item in GIOU]).to(GIOU.device)
...
def smooth_l1_loss(pred,
    ...
    assert pred.shape[-1] == 5, "pred bboxes should be (x1, y1, x2, y2, score)"
    assert target.shape[-1] == 5, "target bboxes should be (x1, y1, x2, y2, score)"
    # print(f"smooth_giou_loss: {pred.shape}, {target.shape}")
...
class SmoothGIoULoss(nn.Module):
    """SmoothGIoULoss.

    Computing the Smooth GIoU loss between a set of predicted rbboxes and
    target rbboxes.
    Args:
    ...
    def __init__(self,
        ...
        super(SmoothGIoULoss, self).__init__()
    ...
    def forward(self,
        ...
        sgl = smooth_giou_loss(pred, target, weight, reduction=reduction, avg_factor=avg_factor, **kwargs)
        # sgl = torch.tensor(sgl).to(pred.device)
        sgl = sgl.clone().detach().requires_grad_(True)
        sll = smooth_l1_loss(pred, target)
        loss = 0.2 * sgl + 0.8 * sll
        return loss
