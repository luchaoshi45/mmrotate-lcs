...
class HITUAVDataset(BaseDataset):
    """HITUAV dataset for detection.

    Note: There are two evaluation methods for HITUAV datasets, which can be
    chosen through ``classwise``. When ``classwise=False``, it means there
    is only one class object; When ``classwise=True``, it means there are 5
    classes.
    ...
    def load_data_list(self) -> List[dict]:
        ...
        assert self._metainfo.get('classes', None) is not None, \
            'classes in `HITUAVDataset` can not be None.'
        if self.classwise:
            self.class2label = {
                cls_id: i
                for i, cls_id in enumerate(self._metainfo['classes'])
            }
        else:
            self._metainfo['classes'] = ('object', )
    ...
    def parse_data_info(self, img_info: dict) -> Union[dict, List[dict]]:
        ...
        for obj in raw_ann_info.findall('object'):
            ...
            if self.classwise:
                ...
                if class_name not in self._metainfo['classes']:
                    continue
            ...
            rbbox = np.array([[
                ...
                float(obj.find('robndbox/angle').text)
            ]],
                             dtype=np.float32)
